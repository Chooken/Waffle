...
namespace WaffleEngine
{
    ...
    public struct Triangle
    {
        ...
        public static bool PointIn(Vector2 vertex_1, Vector2 vertex_2, Vector2 vertex_3, Vector2 point)
        {
            if (point == vertex_1 || point == vertex_2 || point == vertex_3)
                return false;
            
            float d1, d2, d3;
            bool has_neg, has_pos;

            d1 = Sign(point, vertex_1, vertex_2);
            d2 = Sign(point, vertex_2, vertex_3);
            d3 = Sign(point, vertex_3, vertex_1);
        ...
        private static float Sign(Vector2 point_1, Vector2 point_2, Vector2 point_3)
        {
            return (point_1.X - point_3.X) * (point_2.Y - point_3.Y) -
    ...
    public static class BowyerWatson
    {
        ...
        public static Triangle GenSuperTriangle(ref List<Vector2> vertices)
        {
            ...
            foreach (Vector2 vertex in vertices)
            {
                ...
                miny = System.Math.Min(miny, vertex.X);
                maxx = System.Math.Max(maxx, vertex.X);
                maxy = System.Math.Max(maxy, vertex.X);
            }
